                    ...
                    <table id="example2" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$colors->id}}</td>
                                <td>{{$colors->text}}</td>
                                <td class="box-color" style="background-color:{{$colors->value}}">{{$colors->value}}</td>


                                <td>
                                    <button type="button" class="btn btn-outline-primary px-5 mb-2"
                                        data-bs-toggle="modal" data-bs-target="#exampleExtraLargeModal"
                                        onclick="savaData('{{$colors->id}}','{{$colors->text}}','{{$colors->value}}')">Update</button>
                                    <button type="button" class="btn btn-outline-danger px-5 mb-2"
                                        onclick="deleteData('{{$colors->id}}' ,'colors' )">Delete</button>
                                </td>
                            </tr>
                            @endforeach
                            @else
                ...
                <div class="modal fade" id="exampleExtraLargeModal">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <form id="formSubmit" action="{{url('admin/manage_colorUpdate')}}" enctype="multipart/form-data">
                                @csrf
                                    ...
                                    <div class="card-body">
                                        <div class="border p-4 rounded">
                                            <div class="card-title d-flex align-items-center">
                                                ...
                                                <h5 class="mb-0 text-info">Color</h5>
                                            </div>
                                            <hr />
                                            <div class="row mb-3">
                                            ...
                                            <div class="row mb-3">
                                                <label for="inputPhoneNo2" class="col-sm-3 col-form-label">code</label>

                                                <div class="col-sm-9">
                                                    <input type="hidden" class="form-control" id="enter_id" name="id"
                                                        placeholder="Email Address">
                                                    <input type="text" class="form-control" name="code" id="enter_code"
...
<script>
function savaData(id, text, code,) {
    $('#enter_text').val(text);
    $('#enter_code').val(code);
    $('#enter_id').val(id);
